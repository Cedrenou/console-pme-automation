  ...
  return (
    <div className="min-h-screen bg-[#151826] text-white p-8">
      ...
      {services.map(service => (
        <div key={service.lambdaName} className="...">
          <div className="flex items-center justify-between">
            <h2 className="text-xl font-semibold mb-1">{service.displayName}</h2>
          </div>
          <p className="text-gray-400 mb-4">{service.description}</p>
          <Link
            href={`/lambdas/${service.lambdaName}`}
            className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-semibold transition-colors w-full text-center"
          >
            Configurer
          </Link>
        </div>
